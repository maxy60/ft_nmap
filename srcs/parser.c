...
int    parse_arg(int ac, char **av, t_nmap *nmap) {
    ...
    while (i != ac) {
        if(ac == 1 || strcmp(av[i], "--help") == 0) {
        ...
        else if (strcmp(av[i], "--ip") == 0 && i < ac) {
            struct addrinfo hints = {.ai_family = AF_INET};
            struct addrinfo *res;
            if (getaddrinfo(av[i + 1], NULL, &hints, &res)) {
                printf("PRBLM\n");
                return 2;   //gerer l'erreur correctement
            }
            ...
            if (parse_port_range(av[i + 1], &nmap->port_start, &nmap->port_end) != 0) {
                printf("incorrect port range\n");
                return 2;
            }
        }
        else if (strcmp(av[i], "--scan") == 0 && i < ac) {
            if ((nmap->scan_count = parse_scan_types(av[i + 1], nmap->scan_types)) == -1) {
                return 2;
            }
            is_scan_spe = true;
        }
